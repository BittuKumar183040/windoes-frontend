...
interface FolderProps {
  ...
  // Replaces setSelectedNode — parent keeps location[] and selectedNode in sync.
  onRename: (id: string, newName: string) => void;
}

const FileFolder = ({ item, initActive = false, selected, onClick, onDoubleClick, onRename }: FolderProps) => {
  ...
  const renameAPI = async () => {
    ...
    if (value && item.name !== value) {
      try {
        ...
        console.log(e);
        // Roll back local input and notify parent on failure.
  ...
  const handleTextChange = (value: string) => {
    setInputValue(value);
    // Propagate up so location[] and selectedNode stay in sync immediately.
  ...
  return (
    ...
    >
      ...
      <div className="w-full ml-1 mt-2 text-left">
        ...
        />
        <p onClick={handleTextClick} className={`${renameActive ? "hidden" : "block"} text-lg pl-1`}>
          {inputValue}
        </p>
      </div>
    </button>
